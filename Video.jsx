...
const Video = () => {
  ...
  const useManualPayed = () => {
    ...
    return useMutation({
      ...
      onSuccess: (data, { car_number, close }) => {
        ...
        setShiftProfit(prev => prev + payment);
        updateCar({
          plate: car_number,
          reason: "paid",
  ...
  const updateCarHandler = async (carNumber, barrierId, channelId) => {
    ...
    try {
      ...
      const totalPayment = updatedData.total_payment;
      // Добавляем сумму к прибыли оператора
      setOperatorProfit(prev => {
        const newProfit = prev + totalPayment;
        localStorage.setItem("operatorProfit", newProfit);
        return newProfit;
      });
      ...
      setShiftProfit(prev => prev + (totalPayment || 0));
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[1000]">
          <div className="bg-white p-10 rounded-xl w-full max-w-4xl shadow-lg relative">
            ...
            <div className="flex space-x-2 pt-5">
              <button
                onClick={() => {
                  console.log("Opening barrier P3-05 for car:", selectedLog.number);
                  updateCarHandler(
                    selectedLog.number,
                    "P3-05",
                    selectedLog.ChannelId || defaultChannelIds['P3-05']
                  );
      ...
      {modalOpenPayment && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[1000]">
          <div className="bg-white p-10 rounded-xl w-full max-w-4xl shadow-lg relative">
            ...
            <div>
              <img src={selectedPaymentLog?.image_url || "/photo.jpg"} alt="car" className="w-full rounded" />
            </div>
            <div className="flex justify-between space-x-6 pt-5 text-black">
              <div className="text-center">
                <p><FormattedMessage id="video.modal.carNumber" defaultMessage="Car Number" /></p>
                <p className="font-bold">{selectedPaymentLog?.number}</p>
              </div>
              <div className="text-center">
                <p><FormattedMessage id="video.modal.entryTime" defaultMessage="Entry Time" /></p>
                <p className="font-bold">{selectedPaymentLog?.entryTime}</p>
              </div>
              <div className="text-center">
                <p><FormattedMessage id="video.modal.departureTime" defaultMessage="Departure Time" /></p>
                <p className="font-bold">{selectedPaymentLog?.departureTime || intl.formatMessage({ id: "video.modal.notExited", defaultMessage: "Not exited" })}</p>
              </div>
              <div className="flex items-center">
                <h2 className="text-3xl font-semibold text-black">{selectedPaymentLog?.price || intl.formatMessage({ id: "video.error.noTotalPayment", defaultMessage: "Payment not calculated yet, car may still be inside" })}</h2>
              </div>
            </div>
            <div className="flex space-x-2 pt-5">
              <button
                onClick={() => handlePayed(selectedPaymentLog.number, selectedPaymentLog.ChannelId || defaultChannelIds[selectedPaymentLog.parking])}
                className="px-4 py-2 text-green-600 border-2 border-green-500 rounded hover:text-white hover:bg-green-500 w-full mx-1 cursor-pointer"
                disabled={!selectedPaymentLog || !selectedPaymentLog.ChannelId || selectedPaymentLog.price === intl.formatMessage({ id: "video.monthlySubscription", defaultMessage: "Monthly Subscription" }) || selectedPaymentLog.price === intl.formatMessage({ id: "video.error.noTotalPayment", defaultMessage: "Payment not calculated yet, car may still be inside" })}
                style={{ pointerEvents: 'auto' }}
              >
                <FormattedMessage id="video.payed" defaultMessage="Payed" />
              </button>
            </div>
          </div>
        </div>
