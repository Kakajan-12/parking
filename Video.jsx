...
const Video = () => {
  ...
  const updateCarHandler = async (carNumber, barrierId, channelId) => {
    ...
    try {
      ...
      // ✅ Добавляем прибыль только если машина вышла после начала смены и не учтена
      const carExitTime = new Date(updatedData.end_time);
      if (carExitTime > shiftStartRef.current) {
        const lastExit = accountedCarsRef.current.get(car.id);
        if (lastExit !== updatedData.end_time) {
          setShiftProfit((prev) => {
            const updated = prev + (totalPayment || 0);
            localStorage.setItem("shiftProfit", updated);
            return updated;
          });
          accountedCarsRef.current.set(car.id, updatedData.end_time);
          console.log(`💰 Profit added for manual exit: ${car.number}`);
        }
      }
    ...
    const connectWebSocket = () => {
      ...
      ws.onmessage = (event) => {
        ...
        if (data.status === "Exited") {
          const carExitTime = new Date(data.end_time);

          // ✅ Учитываем только машины, вышедшие после начала смены и не учтённые ранее
          if (carExitTime > shiftStartRef.current) {
            const lastExit = accountedCarsRef.current.get(data.id);

            if (lastExit !== data.end_time) {
              setShiftProfit((prev) => {
                const updated = prev + (data.total_payment || 0);
                localStorage.setItem("shiftProfit", updated);
                return updated;
              });
              accountedCarsRef.current.set(data.id, data.end_time);
              console.log(`💰 Profit added for car: ${data.car_number}`);
            }
          }
          setCars((prevCars) =>
            prevCars.map((car) =>
